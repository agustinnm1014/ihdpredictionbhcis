...
app.config['MYSQL_HOST'] = 'IHDpredictionbhcis.mysql.pythonanywhere-services.com'
app.config['MYSQL_USER'] = 'IHDpredictionbhc'
app.config['MYSQL_PASSWORD'] = 'secret123.'
app.config['MYSQL_DB'] = 'IHDpredictionbhc$db_bhcis'
...
@app.before_request
def require_login():
    allowed_routes = ['login', 'register','forgot_password','answer_security_questions','update_password'] # add the allowed routes here
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user['name'] == 'admin' and user['password'] == 'admin123':
...
def register():
    message = ''
    alert_type = ''
    if request.method == 'POST' and 'name' in request.form and 'password' in request.form and 'security_question_1' in request.form and 'security_question_2' in request.form and 'security_answer_1' in request.form and 'security_answer_2' in request.form:
        ...
        # Validating password
            ...
            alert_type = 'danger'
        elif not userName or not password or not security_question_1 or not security_question_2 or not security_answer_1 or not security_answer_2 :
            message = 'Please fill out the form!'
            alert_type = 'danger'
        else:
            cursor.execute('INSERT INTO user (name, password, security_question_1, security_question_2, security_answer_1, security_answer_2) VALUES ( %s, %s, %s, %s, %s, %s)', (userName, password, security_question_1, security_question_2, security_answer_1, security_answer_2))
...
def forgot_password():
    ...
    if request.method == 'POST' and 'name' in request.form:
        ...
        if not user:
            ...
            # User exists, show security questions form
            security_question_1 = user['security_question_1']
            security_question_2 = user['security_question_2']

            return render_template('answer_security_questions.html', name=name, security_question_1=security_question_1, security_question_2=security_question_2)

    return render_template('forgot_password.html', message=message, alert_type=alert_type)

@app.route('/answer-security-questions', methods=['POST'])
def answer_security_questions():
    ...
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('SELECT * FROM user WHERE name = %s AND security_answer_1 = %s AND security_answer_2 = %s', (name, security_answer_1, security_answer_2))
    user = cursor.fetchone()
    ...
    # Incorrect answers, show error message
        ...
        alert_type = 'danger'
        return render_template('answer_security_questions.html', name=name, message=message, alert_type=alert_type)
    else:
    # Correct answers, show form to update password
         return render_template('update_password.html', name=name)
...
def update_password():
    ...
    if not name:
        ...
        alert_type = 'danger'
        return render_template('update_password.html', name=name, message=message, alert_type=alert_type)
...
rf = joblib.load('/home/IHDpredictionbhcis/mysite/model.joblib')
